...
async def extract_entities(
    ...
    # 若未生成任何文本块，直接返回空结果以避免 asyncio.wait 空任务错误
    if not ordered_chunks:
        logger.warning("No chunks provided to extract_entities; skip extraction")
        return []
    ...
    # 当无任务时直接返回，避免 asyncio.wait 抛出空集合错误
    if not tasks:
        logger.warning("No tasks created in extract_entities; skip asyncio.wait")
        return []
